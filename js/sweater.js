...
export const Sweater = async () => {

    try {
        const url = `${localhost}/sweaters/?_sort=id:DESC`;
        const ftch = await fetch(url);
        const data = await ftch.json();
        const result = data;
        // console.log(result[0].sweaterzone[0].desc);
        ...
        for (let i in result) {
            ...
            img.src = `${result[i].sweaterzone[0].img[0].url}`;
            img.classList = "img";

            const name = document.createElement("h1");
            name.textContent = `${result[i].sweaterzone[0].name}`;
            name.classList = "name";

            const piece = document.createElement("p");
            piece.textContent = `${result[i].sweaterzone[0].piece}`;
            piece.classList = "piece";

            const price = document.createElement("p");
            price.textContent = `Price: NPR ${result[i].sweaterzone[0].price}`;
            price.classList = "price";

            const descHeading = document.createElement("h3");
            descHeading.textContent = `${result[i].sweaterzone[0].info}`;
            descHeading.classList = "descHeading";

            const desc = document.createElement("p");
            desc.textContent = `${result[i].sweaterzone[0].desc}`;
            desc.classList = "desc";

            const callheading = document.createElement("p");
            callheading.textContent = `${result[i].sweaterzone[0].callheading}`;
            callheading.classList = "callheading";

            const number = document.createElement("p");
            number.textContent = `${result[i].sweaterzone[0].number}`;
